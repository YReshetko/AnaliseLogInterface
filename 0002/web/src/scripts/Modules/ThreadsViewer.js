...
ali.ThreadsViewer = function(container)
{
	...
	this.loadThreads = function(path)
	{
		this._path = path;
	    var request = ali.CONST.GET_PROCESSED_THREADS;
		request["data"] = path;
		var self = this;
		sendRequest("execute", request, function(response)
		{
			var respObj = JSON.parse(response);
			self.constructThreads(respObj);
		});
	}
	...
	this.removeSelectedThreads = function()
	{
			...
			this._path && this._path.length>0)
		{
			...
			var request = ali.CONST.REMOVE_THREADS;
	...
	this.downloadSelected = function()
	{
		...
		sendRequest("execute", command, function(response){
			if(response.indexOf("CONNECT_TO_PROCESS")==-1)
			{
...
ali.SampleThread = function(thread, backgroundColor, borderColor, path)
{
	...
	this._download = $("<span/>").css(downloadStyle).addClass("glyphicon glyphicon-download-alt");
	...
	this._thread.append(this._threadName);
	...
	Object.defineProperties(this,{
		...
		isWrapp : {
			set : function(value)
			{
				if(value)
				{
					this._line.css('min-height', "22px");
					this._line.css('max-height', "22px");
				}
				else
				{
